...
@app.get("/addresses/{karlsenAddress}/transactions",
...
async def get_transactions_for_address(
        karlsenAddress: str = Path(
            description="Karlsen address as string e.g. "
                        "karlsen:pzhh76qc82wzduvsrd9xh4zde9qhp0xc8rl7qu2mvl2e42uvdqt75zrcgpm00",
    ...
    #
    # WHERE "script_public_key_address" = 'karlsen:qp7d7rzrj34s2k3qlxmguuerfh2qmjafc399lj6606fc7s69l84h7mrj49hu6'
    #
    ...
    async with async_session() as session:
        resp = await session.execute(text(f"""
            ...
            WHERE "script_public_key_address" = :karlsenAddress
			ORDER by transactions.block_time DESC
			LIMIT 500"""),
                                     {'karlsenAddress': karlsenAddress})
...
@app.get("/addresses/{karlsenAddress}/full-transactions",
...
async def get_full_transactions_for_address(
    ...
    async with async_session() as s:
        ...
        tx_within_limit_offset = await s.execute(select(TxAddrMapping.transaction_id)
                                                 .filter(TxAddrMapping.address == karlsenAddress)
                                                 .limit(limit)
                                                 .offset(offset)
...
@app.get("/addresses/{karlsenAddress}/transactions-count",
...
async def get_transaction_count_for_address(
    ...
    async with async_session() as s:
        count_query = select(func.count()).filter(TxAddrMapping.address == karlsenAddress)
