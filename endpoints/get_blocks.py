...
async def get_block(response: Response,
    ...
    """
    resp = await karlsend_client.request("getBlockRequest",
                                       params={
                                           "hash": blockId,
    ...
    if "block" in resp["getBlockResponse"]:
        # We found the block in karlsend. Just use it
        requested_block = resp["getBlockResponse"]["block"]
    else:
        if IS_SQL_DB_CONFIGURED:
            # Didn't find the block in karlsend. Try getting it from the DB
    ...
    # It's possible that the block from karlsend does not contain transactions
...
async def get_blocks(response: Response,
    ...
    the karlsend node. If this is not possible, the database is getting queryied as backup. In this case the response
    ...
    resp = await karlsend_client.request("getBlocksRequest",
                                       params={
                                           "lowHash": lowHash,
...
async def get_blocks_from_bluescore(response: Response,
                                    ...
                                    includeTransactions: bool = False):
    """
    Lists block beginning from a low hash (block id). Note that this function is running on a karlsend and not returning
    data from database.
    """
    response.headers["X-Data-Source"] = "Database"
