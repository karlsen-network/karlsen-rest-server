...
async def get_utxos_for_address(karlsenAddress: str = Path(
    ...
    """
    resp = await karlsend_client.request("getUtxosByAddressesRequest",
                                       params={
                                           "addresses": [karlsenAddress]
                                       }, timeout=120)
    try:
        return (utxo for utxo in resp["getUtxosByAddressesResponse"]["entries"] if utxo["address"] == karlsenAddress)
    except KeyError:
